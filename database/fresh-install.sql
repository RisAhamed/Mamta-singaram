...
-- AK Dental / Mamta Singaram - Fresh Client Database Installation
-- Run once on a brand-new empty CockroachDB database.
-- Usage: psql $DATABASE_URL -f database/fresh-install.sql
...
CREATE TABLE IF NOT EXISTS facial_bones (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT UNIQUE NOT NULL,
  is_active BOOLEAN DEFAULT true,
  is_custom BOOLEAN DEFAULT false,
...
CREATE TABLE IF NOT EXISTS session_surgery_forms (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id UUID NOT NULL REFERENCES sessions(id) ON DELETE CASCADE,
  patient_id UUID NOT NULL REFERENCES patients(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS session_consent_forms (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id UUID NOT NULL REFERENCES sessions(id) ON DELETE CASCADE,
  patient_id UUID NOT NULL REFERENCES patients(id) ON DELETE CASCADE,
